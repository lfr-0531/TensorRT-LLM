...
class Indexer(nn.Module):
    def __init__(
        ...
        indexer_rope_interleave = getattr(sparse_attention_config, "indexer_rope_interleave", False)
        ...
        self.use_cute_dsl_topk = (
            sparse_attention_config.use_cute_dsl_topk and IS_CUTLASS_DSL_AVAILABLE
        )
    ...
    def prepare_one_prefill_chunk(
        ...
        if len(chunk_specs) == 1:
            ...
            req_cached_lens_tensor = torch.tensor(req_cached_lens, dtype=torch.int32, device="cpu")
            ...
            cu_seqlen_ks, cu_seqlen_ke = compute_cu_seqlen_kv_bounds_with_cache(
                req_seq_lens_tensor, len(chunk_specs), num_q_tokens, req_cached_lens_tensor
            )
            ...
            kv_offset_in_extended = metadata.host_ctx_kv_indptr[first_req_idx].item()
            total_kv_len = sum(req_seq_lens_tensor + req_cached_lens_tensor).item()
            k_token_start = kv_offset_in_extended
            k_token_end = kv_offset_in_extended + total_kv_len

        assert cu_seqlen_ks.shape[0] == num_q_tokens == token_end - token_start, (
            f"Indexer.prepare_one_prefill_chunk - cu_seqlen_ks length mismatch: {cu_seqlen_ks.shape[0]} != {num_q_tokens}"  # noqa: E501
        )
        assert cu_seqlen_ke.shape[0] == num_q_tokens == token_end - token_start, (
            f"Indexer.prepare_one_prefill_chunk - cu_seqlen_ke length mismatch: {cu_seqlen_ke.shape[0]} != {num_q_tokens}"  # noqa: E501
        )
    ...
    def prepare(metadata: DSAtrtllmAttentionMetadata):
        ...
        if num_contexts > 0:
            ...
            cached_tokens = metadata.kv_cache_params.num_cached_tokens_per_seq
            host_cached_tokens = torch.tensor(
                cached_tokens[:num_contexts], dtype=torch.int32, device="cpu"
            )
            ...
            if has_mla_chunked_prefill:
                ...
                chunk_specs = [
                    (i, 0, host_seq_lens[i].item(), host_seq_lens[:i].sum().item() if i > 0 else 0)
                ...
                # Use indexer's own chunking logic to prevent L^2 complexity of indexer MQA logits computation for long sequences.  # noqa: E501
            ...
            metadata.cu_seqlen_ks[:num_ctx_tokens].copy_(host_cu_seqlen_ks, non_blocking=True)
            metadata.cu_seqlen_ke[:num_ctx_tokens].copy_(host_cu_seqlen_ke, non_blocking=True)
        ...
        # When chunked prefill or KVCache reuse is enabled, we need to gather the full KV for indexer's logit computation.  # noqa: E501
        # Indexer's own chunking does not need full KV gathering, instead it gathers only the current chunk with loop-based gathering.  # noqa: E501
        ...
        if _need_full_kv_gathering:
            ...
            total_kv_per_request = seq_lens[:num_contexts] + start_positions[:num_contexts]
            ...
            kv_positions = torch.cat(
                [
                    torch.arange(total_kv_per_request[i].item(), dtype=torch.int64, device="cpu")
                    for i in range(num_contexts)
                ]
            )
            ...
            assert len(fp8_flat_indices) == total_kv_len, (
                f"host_slot_mapping_fp8_fullkv/host_slot_mapping_scale_fullkv length mismatch: {len(fp8_flat_indices)} != total_kv_len={total_kv_len}"  # noqa: E501
            )
    ...
    def _update_k_cache(
        ...
        torch.ops.trtllm.indexer_k_cache_scatter_op(
            k_fp8_bytes, k_scale_bytes, k_cache, flat_indices_fp8, flat_indices_scale
        )
    ...
    def sparse_attn_indexer(
        ...
        if has_prefill and not metadata.skip_indexer_for_ctx_reqs:
            ...
            if metadata.indexer_prefill_chunks is not None:
                ...
                q_split_threshold = (
                    metadata.sparse_attention_config.q_split_threshold
                    if metadata.sparse_attention_config is not None
                    else 8192
                )
                q_split_eligible = (
                    q_split_threshold >= 0
                ...
                for chunk in metadata.indexer_prefill_chunks:
                    ...
                    if use_custom_topk:
                        ...
                        topk_indices = logits.topk(min(self.index_topk, logits.shape[-1]), dim=-1)[
                            1
                        ]
                        topk_indices -= chunk.cu_seqlen_ks[chunk_q_start:chunk_q_end][:, None]
                        ...
                        topk_indices_buffer[
                            global_q_start:global_q_end, : topk_indices.shape[-1]
                ...
                if use_custom_topk:
                    torch.ops.trtllm.indexer_topk_prefill(
                        logits, cu_seqlen_ks, cu_seqlen_ke, topk_indices_buffer[:num_ctx_tokens, :]
                    )
                else:
        ...
        if has_decode and not metadata.skip_indexer_for_gen_reqs:
            ...
            if use_custom_topk:
                ...
                # padded
                positions = (
                    torch.arange(max_seq_len, device=q_decode.device)
                    .unsqueeze(0)
            ...
            # Fill topk_indices_buffer with pre-defined dense topk indices
            topk_indices_buffer[num_ctx_tokens:num_tokens, :] = metadata.topk_indices_buffer[
    ...
    def pre_indexer_proj(
        ...
        fused_out = torch.ops.trtllm.cublas_mm(
            hidden_float, self._fused_wk_wp_weight.t(), None, out_dtype=None
        )
