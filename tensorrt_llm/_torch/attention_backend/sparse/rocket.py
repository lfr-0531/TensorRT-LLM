...
class RocketVanillaAttention(VanillaSparseAttention):
    ...
    def single_request_sparse_kv_predict(self,
        ...
        selected_indices = self._get_snapkv_indices(q, k, window_size, kernel_size)
    ...
    def _get_snapkv_indices(self, q: Tensor, k: Tensor, kernel_size: int) -> Tensor:
        ...
        q_obs = q[:,:,-self.window_size:] # (1, num_kv_heads, window_size, head_dim)
        k_pre = repeat_kv(k[:,:,:-self.window_size], self.num_key_value_groups) # (1, num_heads, seq_len-window_size, head_dim)
        ...
        selected_prefix_indices = score.topk(self.prompt_budget-self.window_size, dim=-1).indices.sort().values
    ...
    def single_request_sparse_attn_predict(self, 
        ...
        calc_indices = self._rocketkv_selection(q, k, metadata, past_seen_token, cache_idx)
        return calc_indices
    ...
    def _rocketkv_selection(self, q: Tensor, k: Tensor, metadata: SparseAttentionMetadata, 
        ...
        i1 = torch.topk(qi_abs.sum(dim=2, keepdim=True), self.topr, dim=-1).indices
