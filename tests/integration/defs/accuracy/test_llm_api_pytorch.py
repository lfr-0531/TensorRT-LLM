...
class TestDeepSeekV32(LlmapiAccuracyTestHarness):
    ...
    @pytest.mark.parametrize(
        ...
        ids=["baseline", "baseline_mtp1", "baseline_fp8kv"])
    def test_nvfp4(self, tp_size, pp_size, ep_size, mtp_nextn, fp8kv,
                   attention_dp, cuda_graph, overlap_scheduler, max_batch_size,
                   moe_backend):
        if get_sm_version() == 100 or get_sm_version() == 103:
            moe_backend = "TRTLLM" if moe_backend == "_DEFAULT" else moe_backend
            moe_config = MoeConfig(backend=moe_backend, max_num_tokens=16384)
            # TODO: Support block reuse for DeepSeek-V3.2
            kv_cache_config = KvCacheConfig(enable_block_reuse=False,
                                            free_gpu_memory_fraction=0.6)
