...
TEST_F(sparseAttentionUtilsTest, GatherKvPageOffsetsKernelTest)
{
    ...
    auto output_seq_lengths = mBufferManager->gpu(ITensor::makeShape({batch_size}), nvinfer1::DataType::kINT32);
    ...
    auto kv_page_offsets_ptr = bufferCast<int>(*kv_page_offsets_host);
    ...
    seq_lengths_ptr[0] = 3 * tokens_per_page;     // 3 pages for batch 0
    seq_lengths_ptr[1] = 3 * tokens_per_page + 1; // 4 pages for batch 1
    ...
    int num_sparse_pages = 4;
    int sparse_page_indices[4][4] = {{1, 0, 0, 1}, {2, 2, 2, 2}, {0, 1, 2, 3}, {3, 3, 3, 3}};
    ...
    sparse_indices_offsets_ptr[1] = 2; // Start of batch 1 (3 sparse pages for batch 0)
    sparse_indices_offsets_ptr[2] = 4; // End (3 sparse pages for batch 1)
    ...
    // Launch the kernel
    invokeGatherKvPageOffsets(bufferCast<int>(*output_kv_page_offsets), bufferCast<int>(*output_seq_lengths),
        bufferCast<int>(*kv_page_offsets), bufferCast<int>(*seq_lengths), sparse_params, mStream->get());
    ...
    auto output_kv_page_offsets_host = mBufferManager->pinned(
        ITensor::makeShape({num_head_kv, batch_size, 2, max_num_pages_per_seq}), nvinfer1::DataType::kINT32);
    auto output_seq_lengths_host = mBufferManager->pinned(ITensor::makeShape({batch_size}), nvinfer1::DataType::kINT32);
    ...
    auto output_kv_offsets_ptr = bufferCast<int>(*output_kv_page_offsets_host);
    auto output_seq_len_ptr = bufferCast<int>(*output_seq_lengths_host);

    // Verify sequence lengths (should remain the same since we're using exact page boundaries)
    EXPECT_EQ(output_seq_len_ptr[0], seq_lengths_ptr[0] - tokens_per_page);
    EXPECT_EQ(output_seq_len_ptr[1], seq_lengths_ptr[1] - tokens_per_page * 2);
    ...
    for (int h = 0; h < num_head_kv; ++h)
    {
        for (int b = 0; b < batch_size; ++b)
        {
            int num_sparse_pages = sparse_indices_offsets_ptr[b + 1] - sparse_indices_offsets_ptr[b];

            for (int d = 0; d < 2; ++d)
            {
                for (int p = 0; p < num_sparse_pages; ++p)
                {
