...
void MLACacheFormatter::format(tensorrt_llm::batch_manager::TransferSession& session)
{
    ...
    auto numPools = mCacheManager->getBlockManager().getNumPools(
    ...
    for (auto transferIndexerKCache : transferringIndexerKCache)
    {
        ...
        {
            auto blockRangeForWindow = blockRange.getBlockRangeForWindow(windowSizes[0], true);
            for (auto it = blockRangeForWindow.begin(); it != blockRangeForWindow.end(); ++it)
            {
...
void MLACacheFormatter::unformat(tensorrt_llm::batch_manager::TransferSession& session)
{
    ...
    for (auto transferIndexerKCache : transferringIndexerKCache)
    {
        ...
        else
        {
            ...
            {
                TLLM_CHECK_WITH_INFO(preAllocRecvBuffer->getDataType() == outputBuffers.at(0)->getDataType(),
                    "%d (preAllocRecvBuffer) != %d (outputBuffers.at(0))", preAllocRecvBuffer->getDataType(),
                    outputBuffers.at(0)->getDataType());
            }
